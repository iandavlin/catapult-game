...
const FW=1400,FH=800,VW=900,VH=600;
const BR=11,BS=28,MP=100;
const MT=100,MK=1200;
const ZM=VW/FW,FLOOR_Y=FH-14,CEIL_Y=14;
const CL=35,CW=14,CANNON_STEP=20;
...
const DEF={gv:0.15,bnc:0.78,fr:0.999,ff:0.95,sc:0.055,sb:0.7,pm:0.34,
  ...
  roundColored:3,winScore:500,tankKillPts:100,maxAmmo:3,puSpread:5};
...
// Team colors by point value
const P1C=["#1a3d6e","#2255aa","#3377cc","#4499ee","#55bbff"];
const P2C=["#6e1a1a","#aa3322","#cc5544","#ee6655","#ff8877"];
const GREY_COLS=["#2e3848","#4a5a6a","#7a8a9a"]; // dark, medium, light (by remaining hits)
...
function nearBucket(r){return false;} // no buckets
...
let debris=[];let floatTexts=[]; // {x,y,text,color,life}
...
function addRoundBlocks(bl,cfg,round,diff){
  ...
  for(let i=0;i<needG;i++){const res=placeGreyLeft(bl,sL,centerX-BS,floorY,ceilY,cfg.greyHits);
    if(res){const mr=mirrorRects(res.rects);const gR=nGid();let ok=true;
      for(const r of mr){if(overlaps(r,bl)){ok=false;break;}}
      if(ok)for(const r of mr)bl.push({...r,color:greyCol(cfg.greyHits,cfg.greyHits),player:-1,id:Math.random(),groupId:gR,isGrey:true,vy:0,hits:cfg.greyHits});}}
  // Colored: roundColored + diff extra on the side that broke more
  const shadow=[...bl];const base=cfg.roundColored;
  // diff>0 means P1 broke more, so extra blocks go on P1's side (left)
    ...
    for(let t=0;t<80;t++){const px=Math.round((sL+Math.random()*(centerX-BS-sL-BS*5))/BS)*BS;
      ...
      if(d){let ok=true;for(const r of d){if(r.x<sL||r.x+r.w>centerX-BS){ok=false;break;}}
        ...
        if(ok){const gL=nGid(),gR=nGid(),cL=teamCol(0),cR=teamCol(1);
          ...
          break;}}}}
  // Extra blocks for the side that broke more
...
function fb(pi,a,pm,cx,cy,face,maxVel){
  const rad=a.angle*Math.PI/180;const spd=Math.min(a.power*pm,maxVel);
  let dx,dy;
  if(face==="up"){dx=Math.cos(rad);dy=-Math.sin(rad);}
  else if(face==="down"){dx=Math.cos(Math.PI-rad);dy=Math.sin(rad);}
  else if(face==="left"){dx=-Math.sin(rad);dy=-Math.cos(rad);}
  else{dx=Math.sin(rad);dy=-Math.cos(rad);}
  return{x:cx+dx*CL,y:cy+dy*CL,vx:dx*spd,vy:dy*spd,spin:a.spin,trail:[],player:pi,scored:false,settled:false,rot:0,stuckTicks:0,bounces:0,streak:0};}

// Tank perimeter: 0->FW(bottom), FW->FW+FH(right wall), FW+FH->2FW+FH(ceiling), 2FW+FH->perim(left wall)
const PERIM=2*FW+2*(FH-28);
function getTankPos(pos){
  const d=((pos%1)+1)%1*PERIM; // normalize 0-1
  if(d<FW){return{x:d,y:FLOOR_Y,face:"up",ang:0};} // bottom: face up
  if(d<FW+(FH-28)){return{x:FW,y:FLOOR_Y-(d-FW),face:"left",ang:Math.PI/2};} // right wall: face left
  if(d<2*FW+(FH-28)){return{x:FW-(d-FW-(FH-28)),y:CEIL_Y,face:"down",ang:Math.PI};} // ceiling: face down
  return{x:0,y:CEIL_Y+(d-2*FW-(FH-28)),face:"right",ang:-Math.PI/2};} // left wall: face right

function init(cfg){debris=[];floatTexts=[];gid=0;const c=cfg||DEF;const bl=genField(c);
  const{blocks:bl2,drops}=addRoundBlocks(bl,c,0,0);
  const fp=Math.random()<0.5?"p1":"p2";const ao=[fp,fp==="p1"?"p2":"p1"];
  const base={scores:[0,0],blocks:bl2,aims:[{angle:55,power:60,spin:0},{angle:55,power:60,spin:0}],
    tankPos:[130/PERIM,(PERIM/2+130)/PERIM],cannonDisabled:[false,false],balls:[],round:1,
...
export default function CG(){
  ...
  const draw=useCallback(()=>{
    ...
    x.save();x.fillStyle="#050508";x.fillRect(0,0,VW,VH);x.scale(ZM,ZM);
    ...
    // Cannons on perimeter
    for(let pi=0;pi<2;pi++){
      const tp=getTankPos(g.tankPos[pi]);const col=PC[pi],li=PL[pi];
      const label=["P1","P2"][pi];const angle=g.aims[pi].angle;const power=g.aims[pi].power;
      const active=(g.phase==="p1aim"&&pi===0)||(g.phase==="p2aim"&&pi===1);
      const disabled=g.cannonDisabled[pi];
      x.save();x.translate(tp.x,tp.y);x.rotate(tp.ang);
      x.globalAlpha=disabled?0.3:1;
      // Base (sits on surface, extends inward)
      x.fillStyle="#333";x.fillRect(-22,-22,44,22);x.fillStyle="#444";x.fillRect(-18,-18,36,14);
      x.fillStyle="#555";x.beginPath();x.arc(0,-22,10,0,Math.PI*2);x.fill();
      // Barrel (aim relative to surface normal)
      const rad=angle*Math.PI/180;x.save();x.translate(0,-22);x.rotate(-rad);
      x.shadowColor=active?li:"transparent";x.shadowBlur=active?8:0;
      x.fillStyle=col;x.fillRect(0,-CW/2,CL,CW);x.fillStyle="#777";x.fillRect(CL-4,-CW/2-2,6,CW+4);
      x.shadowBlur=0;x.restore();
      // Power meter
      const mW=54,mH=7;x.fillStyle="#0a0a0a";x.fillRect(-mW/2,3,mW,mH);const pc=power/MP;
      let cr,cg2;if(pc<.5){cr=Math.round(pc*2*255);cg2=255;}else{cr=255;cg2=Math.round((1-(pc-.5)*2)*255);}
      x.fillStyle=`rgb(${cr},${cg2},0)`;x.fillRect(-mW/2+1,4,(mW-2)*pc,mH-2);
      // Label
      x.fillStyle="#fff";x.font="bold 12px monospace";x.textAlign="center";x.fillText(label,0,-38);
      if(disabled){x.fillStyle="#ff0000";x.font="bold 10px monospace";x.fillText("DISABLED",0,-50);}
      // Ammo badge
      const am=g.ammo?g.ammo[pi]:1;
      if(am>1){x.fillStyle="#ff00ff";x.font="bold 10px monospace";x.fillText("\u00d7"+am,0,18);}
    ...
    x.restore();
    // HUD
    x.fillStyle=PC[0];x.font="bold 32px monospace";x.textAlign="center";x.fillText(g.scores[0],42,VH/2+8);
    x.fillStyle=PC[0]+"44";x.font="9px monospace";x.fillText("P1",42,VH/2+24);
    x.fillStyle=PC[1];x.font="bold 32px monospace";x.fillText(g.scores[1],VW-42,VH/2+8);
    x.fillStyle=PC[1]+"44";x.font="9px monospace";x.fillText("P2",VW-42,VH/2+24);
    x.fillStyle="#444";x.font="8px monospace";x.fillText("first to "+P.winScore,VW/2,VH-2);
    x.fillStyle="#333";x.font="9px monospace";x.textAlign="center";x.fillText("R"+g.round+(g.volley>1?" V"+g.volley:""),VW/2,14);
    const ap=g.phase==="p1aim"?0:g.phase==="p2aim"?1:-1;
    if(ap>=0){x.fillStyle=PC[ap];x.font="bold 11px monospace";x.fillText("\u25bc "+["P1","P2"][ap]+" \u25bc",VW/2,28);}
    if(g.phase==="p1aim"||g.phase==="p2aim"){const pi=ap,a=g.aims[pi];
      const smX=VW/2-50,smY=VH-38,smW=100,smH=10;
      x.fillStyle="rgba(0,0,0,0.6)";x.fillRect(smX-5,smY-16,smW+10,32);x.fillStyle="#555";x.font="8px monospace";x.textAlign="center";x.fillText("SPIN  < >",VW/2,smY-4);
      x.fillStyle="#111";x.fillRect(smX,smY,smW,smH);x.fillStyle="#333";x.fillRect(smX+smW/2-1,smY,2,smH);
      if(a.spin>0){x.fillStyle="#ff8800";x.fillRect(smX+smW/2,smY,(smW/2)*a.spin,smH);}
      else if(a.spin<0){x.fillStyle="#00ddff";x.fillRect(smX+smW/2+(smW/2)*a.spin,smY,(smW/2)*(-a.spin),smH);}
      x.strokeStyle="#444";x.lineWidth=1;x.strokeRect(smX,smY,smW,smH);}
    if(g.msg){x.fillStyle="rgba(0,0,0,0.5)";x.fillRect(VW/2-200,VH-22,400,16);
      x.fillStyle=g.msg.includes("KILL")?"#ff4444":g.msg.includes("FIRE")?"#ff9800":"#666";
      x.font="9px monospace";x.textAlign="center";x.fillText(g.msg,VW/2,VH-10);}
  },[]);
  ...
  useEffect(()=>{
    const step=()=>{rf.current=requestAnimationFrame(step);const g=sR.current;const P=phR.current;
      ...
      // Tank collision using perimeter position
      const cd=[g.cannonDisabled[0],g.cannonDisabled[1]];const ns=[...g.scores];const tp2=[...g.tankPos];
      for(let pi=0;pi<2;pi++){if(cd[pi])continue;const tp=getTankPos(tp2[pi]);
        const tr={x:tp.x-22,y:tp.y-22,w:44,h:22};
        // Adjust hitbox for wall/ceiling
        if(tp.face==="left"||tp.face==="right"){tr.x=tp.x-11;tr.y=tp.y-22;tr.w=22;tr.h=44;}
        if(tp.face==="down"){tr.y=tp.y;}
        // Block crush
        for(const b of g.blocks){if(!b.isBarrier&&b.x<tr.x+tr.w&&b.x+b.w>tr.x&&b.y<tr.y+tr.h&&b.y+b.h>tr.y){
          cd[pi]=true;tp2[pi]=pi===0?130/PERIM:(PERIM/2+130)/PERIM;ns[1-pi]+=P.tankKillPts;
        ...
        if(!cd[pi])for(const ball of g.balls){if(ball.settled)continue;
          const dx=ball.x-Math.max(tr.x,Math.min(ball.x,tr.x+tr.w)),dy=ball.y-Math.max(tr.y,Math.min(ball.y,tr.y+tr.h));
          if(dx*dx+dy*dy<BR*BR){cd[pi]=true;tp2[pi]=pi===0?130/PERIM:(PERIM/2+130)/PERIM;ns[1-pi]+=P.tankKillPts;
      ...
      const nb=g.balls.map(b=>{if(b.settled)return b;
        ...
        // Block collision: own side=bounce, opponent side=break single block
        ...
        for(let bi=0;bi<g.blocks.length;bi++){
          ...
          if(isOwn)break; // own side: bounce only
          // Opponent side: single block destruction
          ...
          const pts=Math.pow(2,sk);sk++;bbk[player]++;
          bpAdd[player]+=pts;
          ...
          const ptStr=pts>=8?"+"+pts+"!!":pts>=4?"+"+pts+"!":"+"+pts;
          ...
          if(hb.hasPowerup){puAmmo[player]++;
            ...
            // Spawn second ball at impact, same velocity, +-5deg spread, fresh bounces
      ...
      set(p=>({...p,balls:combined,blocks:newBl,fireTicks:ticks,cannonDisabled:cd,tankPos:tp2,scores:ns,pendingPts:pp,ammo:newAmmo,blocksBroken:newBB}));

      if(allS&&combined.every(b=>b.settled)){
        if(g.shotQ&&g.shotQ.length>0){const next=g.shotQ[0];const P2=phR.current;
          const tp=getTankPos(sR.current.tankPos[next.pi]);const ball=fb(next.pi,next.aim,P2.pm,tp.x,tp.y-22,tp.face,P2.maxBallVel);
        ...
        setTimeout(()=>{set(p=>{
          ...
          const diff=finalBB[0]-finalBB[1]; // positive=P1 broke more
          ...
          if(drops.length>0)return{...p,balls:[],scores:fs,fireTicks:0,pendingPts:[0,0],blocksBroken:[0,0],round:nr,volley:nv,
            cannonDisabled:[false,false],blocks:nb2,aims:[{angle:55,power:60,spin:0},{angle:55,power:60,spin:0}],
  ...
  useEffect(()=>{
    ...
    const wp=(e)=>{const r=c.getBoundingClientRect();return{x:(e.clientX-r.left)*(VW/r.width)/ZM,y:(e.clientY-r.top)*(VH/r.height)/ZM};};
    const onMM=(e)=>{const g=sR.current;
      for(let pi=0;pi<2;pi++){
        if(g.phase===(pi===0?"p1aim":"p2aim")&&!g.cannonDisabled[pi]){
          const p=wp(e);const tp=getTankPos(g.tankPos[pi]);
          // Angle relative to surface normal
          const dx=p.x-tp.x,dy=p.y-tp.y;
          const localAng=Math.atan2(dx,dy); // angle from surface inward
          const raw=(-localAng+tp.ang)*180/Math.PI+90;
          set(s=>{const a=[...s.aims];a[pi]={...a[pi],angle:Math.max(0,Math.min(180,raw))};return{...s,aims:a};});}}};
    const onClick=(e)=>{const g=sR.current;const P=phR.current;
      ...
      if(g.phase===second+"aim"&&e.button===0){
        ...
        {const tp=getTankPos(g.tankPos[0]);if(!g.cannonDisabled[0])balls.push(fb(0,g.aims[0],P.pm,tp.x,tp.y-22,tp.face,P.maxBallVel));}
        {const tp=getTankPos(g.tankPos[1]);if(!g.cannonDisabled[1])balls.push(fb(1,g.aims[1],P.pm,tp.x,tp.y-22,tp.face,P.maxBallVel));}
        ...
        if(balls.length===0){set(s=>({...s,msg:"BOTH DISABLED!"}));setTimeout(()=>set(s=>({...s,cannonDisabled:[false,false],tankPos:[130/PERIM,(PERIM/2+130)/PERIM],phase:first+"aim",msg:"Cannons restored!"})),1000);}
    ...
    const onKD=(e)=>{const g=sR.current;
      if(g.phase==="p1aim"||g.phase==="p2aim"){const pi=g.phase==="p1aim"?0:1;
        const step=CANNON_STEP/PERIM;
        // P1: 0-0.5, P2: 0.5-1.0
        const minP=pi===0?0:0.5;const maxP=pi===0?0.5:1.0;
        if(e.code==="ArrowLeft"){e.preventDefault();set(s=>{const tp=[...s.tankPos];tp[pi]=Math.max(minP,tp[pi]-step);return{...s,tankPos:tp};});}
        if(e.code==="ArrowRight"){e.preventDefault();set(s=>{const tp=[...s.tankPos];tp[pi]=Math.min(maxP,tp[pi]+step);return{...s,tankPos:tp};});}
      ...
      if(e.code==="Space"&&g.phase==="gameover"){e.preventDefault();debris=[];floatTexts=[];set(init(phR.current));}};
    c.addEventListener("mousemove",onMM);c.addEventListener("click",onClick);
    c.addEventListener("wheel",onWh,{passive:false});document.addEventListener("keydown",onKD);
    c.addEventListener("contextmenu",e=>e.preventDefault());
    return()=>{c.removeEventListener("mousemove",onMM);c.removeEventListener("click",onClick);c.removeEventListener("wheel",onWh);document.removeEventListener("keydown",onKD);};
  },[set]);
  ...
  return(
    <div className="flex flex-col items-center justify-center min-h-screen bg-black select-none" style={{fontFamily:F}}>
      <canvas ref={cv} width={VW} height={VH} className="block rounded" style={{maxWidth:"100%",height:"auto",cursor:gs.phase.includes("aim")?"crosshair":"default"}}/>
      ...
      {showDbg&&(
        <div style={{position:"fixed",top:36,right:8,zIndex:50,background:"rgba(0,0,0,0.92)",border:"1px solid #333",borderRadius:6,padding:"8px 10px",display:"flex",flexDirection:"column",gap:3,maxHeight:"92vh",overflowY:"auto",width:280}}>
          ...
          <div style={{display:"flex",gap:6,marginTop:4}}>
            <button onClick={()=>{debris=[];floatTexts=[];set(init(phR.current));}} style={{flex:1,background:"#333",color:"#4caf50",border:"1px solid #4caf50",padding:"3px 6px",fontSize:10,fontFamily:"monospace",cursor:"pointer",borderRadius:3}}>NEW GAME</button>
      ...
      <div className="mt-2 text-xs text-gray-700 text-center" style={{fontFamily:F}}>
        {(phase==="p1aim"||phase==="p2aim")&&"\u2190\u2192 move \u00b7 MOUSE aim 180\u00b0 \u00b7 SCROLL power \u00b7 < > spin \u00b7 CLICK lock"}
      </div>
    </div>);
}
